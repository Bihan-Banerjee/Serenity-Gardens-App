...
const SEED_REVIEWS: Review[] = [
  {
    _id: 's1',
    name: 'Priya Sharma',
    message: 'An absolute gem! The lotus pond at dawn was the most peaceful thing I\'ve ever experienced.',
    rating: 5,
  ...
  },
  {
    _id: 's2',
    name: 'Rohan Ghosh',
    message: 'Visited last monsoon — the entire place transformed into something magical.',
    rating: 5,
  ...
  },
  {
    _id: 's3',
    name: 'Ananya Bose',
    message: 'Ordered the rose bouquet for my anniversary — delivered fresh and fragrant.',
    rating: 4,
...
function TestimonialMarquee({ reviews }: { reviews: Review[] }) {
  ...
  const CARD_W = 260;
  const totalW = (reviews.length || 1) * (CARD_W + 16);
        ...
        {allReviews.map((review, i) => (
          <View key={`${review._id}-${i}`} style={[marqueeStyles.card, { width: CARD_W }]}>
            ...
            <Text style={marqueeStyles.message} numberOfLines={4}>
              "{review.message || (review as any).review}"
            </Text>
...
function ReviewCard({ review, index }: { review: Review; index: number }) {
  ...
  return (
    ...
    >
      ...
      <Text style={reviewStyles.message}>"{review.message || (review as any).review}"</Text>
    </Animated.View>
  );
}
...
export default function ReviewsScreen() {
  ...
  const fetchReviews = async () => {
    try {
      ...
      console.log('Failed to load reviews from API, using seed data');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      // 2. Map payload to backend expectations: message -> review
      await reviewsAPI.create({ 
        name: name.trim(), 
        review: message.trim(), // Backend expects "review"
        rating 
      });
      ...
      // Refresh the list to show new review from DB
      fetchReviews();
      
    } catch (err: any) {
      Alert.alert('Submission Failed', err.response?.data?.message || 'Could not submit review at this time.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false} bounces>
      ...
      <LinearGradient colors={[Colors.primary, Colors.primaryDark]} style={styles.banner}>
        ...
        <Text style={styles.bannerSub}>Hear what visitors say about Serenity Gardens</Text>
        <View style={styles.statsRow}>
          {[
            { label: 'Reviews', value: reviews.length },
            {
              label: 'Avg Rating',
              value: (reviews.reduce((s, r) => s + (r.rating || 5), 0) / (reviews.length || 1)).toFixed(1) + ' ★',
            },
            { label: '5-Star', value: reviews.filter((r) => (r.rating || 5) === 5).length },
          ].map((stat) => (
      ...
      <View style={styles.formSection}>
        ...
        {submitted ? (
          <View style={styles.successBox}>
            ...
            <Text style={styles.successSub}>Your review has been successfully posted.</Text>
            <Pressable style={styles.anotherBtn} onPress={() => setSubmitted(false)}>
...
const styles = StyleSheet.create({
  ...
  sectionEyebrow: { fontSize: 11, color: Colors.secondary, letterSpacing: 2.5, textTransform: 'uppercase', fontWeight: '700', marginBottom: 4 },
  sectionTitle: { fontSize: 22, fontWeight: '800', color: Colors.text, letterSpacing: -0.4, marginBottom: 16 },
  formCard: { backgroundColor: Colors.card, borderRadius: Theme.borderRadius.xl, padding: 20, borderWidth: 1, borderColor: Colors.cardBorder, ...Theme.shadow.md },
  formLabel: { fontSize: 12, fontWeight: '700', color: Colors.textSecondary, marginBottom: 8, letterSpacing: 0.4, textTransform: 'uppercase' },
  ...
  successBox: { backgroundColor: Colors.card, borderRadius: Theme.borderRadius.xl, padding: 32, alignItems: 'center', borderWidth: 1, borderColor: Colors.cardBorder, ...Theme.shadow.md },
...
const marqueeStyles = StyleSheet.create({
  container: { overflow: 'hidden', paddingVertical: 20, backgroundColor: Colors.backgroundDark },
  track: { flexDirection: 'row', paddingHorizontal: 16 },
  card: { backgroundColor: Colors.card, borderRadius: Theme.borderRadius.lg, padding: 18, marginRight: 16, borderWidth: 1, borderColor: Colors.cardBorder, ...Theme.shadow.sm },
  stars: { flexDirection: 'row', gap: 3, marginBottom: 10 },
  star: { fontSize: 15 },
  starFilled: { color: Colors.accent },
  starEmpty: { color: Colors.cardBorder },
  message: { fontSize: 13, color: Colors.textSecondary, lineHeight: 19, fontStyle: 'italic', flex: 1, marginBottom: 12 },
...
const reviewStyles = StyleSheet.create({
  card: { backgroundColor: Colors.card, borderRadius: Theme.borderRadius.lg, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: Colors.cardBorder, ...Theme.shadow.sm },
  ...
  ratingBadge: { backgroundColor: Colors.accent + '22', borderRadius: Theme.borderRadius.full, paddingHorizontal: 8, paddingVertical: 3 },
  ratingText: { color: Colors.accent, fontSize: 12, fontWeight: '700' },
  message: { fontSize: 14, color: Colors.textSecondary, lineHeight: 21, fontStyle: 'italic' },
});
